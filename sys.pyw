...
class Login(Tk):
    ...
    def Login(self):
        ...
        if self.username.get() != 'root':
            messagebox.showerror('Error','The Username or Password is correct')
        elif self.password.get() != '1915':
            messagebox.showerror('Error','The Username or Password is correct')
        else:
            Login_ = True
            self.destroy()
    def Buttons(self):
        Submit = CTkButton(self,text='Submit',command=self.Login,border_color='red')
        Submit.place(x=265,y=230)
        Submit.bind('<Button-1>',lambda s:Submit.configure(hover_color='blue'))
    def delete(self,e):
        widget = e.widget
        widget.delete(0,END)
    def PassInser(self,e):
        widget = self.password
        if widget.get() == '':
            widget.insert(0, 'Password')
    def UserInser(self,e):
        widget = self.username
        if widget.get() == '':
            widget.insert(0, 'Username')
    def Logo(self):
        self.img = ImageTk.PhotoImage(Image.open('1676750364225.png').resize((160,140)))
        self.la = Label(self,image=self.img,bg='silver')
        self.la.place(x=30,y=80)
    def lines(self):
        lineone = Frame(self,width=182,height=2,bg='black')
        linetwo = Frame(self,width=182,height=2,bg='black')
        lineone.place(x=250,y=120)
        linetwo.place(x=250,y=170)
    def Entrys(self):
        self.username = Entry(self,border=0,bd=0,justify='center',bg='silver'
        ,font='bold')
        self.password = Entry(self,border=0,bd=0,justify='center',bg='silver'
        ,font='bold')
        self.username.place(x=250,y=100)
        self.password.place(x=250,y=150)
        self.username.insert(0, 'Username')
        self.password.insert(0, 'Password')
        self.username.bind('<Enter>',self.delete)
        self.password.bind('<Enter>',self.delete)
        self.password.bind('<Leave>',self.PassInser)
        self.username.bind('<Leave>',self.UserInser)
    def __init__(self):
        super().__init__()
